...
# ╔═╡ fb0d2120-1397-11f0-396d-65684b386889
begin
	using CairoMakie, AlgebraOfGraphics
	using JSON, CSV, DataFrames, Chain, DelimitedFiles
	using Statistics
end

# ╔═╡ 5f087459-3a11-40a0-a040-37ca7be45c36
using NewickTree, BasicTreePlots

# ╔═╡ efe270e9-5f20-4e69-97de-8fed8ac5996c
using ColorSchemes, Colors

# ╔═╡ d8b58cf3-6748-4aaa-891f-a9c67cef3bce
cpd2ma = @chain CSV.read("../data/bash_MA_output/complete_MAs.csv", DataFrame) begin
	transform(:ma => ByRow(x-> x == "na" ? missing : parse(Int, x)) => :ma)
	eachrow
	Dict
end

# ╔═╡ afb4c0a6-584b-49a9-a062-c067e3939850
taxonomy = CSV.read("../bin/kegg-small/data/kegg-small.tsv", DataFrame)

# ╔═╡ a9c1d56c-cdfe-43f1-929d-d32f3746a9b1
...
# ╔═╡ 1568349e-d7bf-4eaf-ba81-04ac7641b9a8
begin
	df = []
	for org in taxonomy[!, :Organism]
		lookup = vec(readdlm("../bin/kegg-small/data/lookup/$org.lookup_latest.txt"))
		filter!(x -> startswith(x, "cpd:"), lookup)
		lookup = map(x -> replace(x, "cpd:" => ""), lookup)
		for cpd in lookup
			if !isnothing(cpd2ma[cpd]) && !ismissing(cpd2ma[cpd])
			push!(
				df,
				merge(
					Dict(
...
sort!(df, by=x->(x["Superkingdom"], x["Kingdom"], x["Genus"],x["ma"]), rev=true)
...
# ╔═╡ 67002b37-4d07-46e2-9253-cef108256434
...
begin
	...
	ylims!(f1a1, (-1, last(argmax(last, leaves_xy))+1))
...
manifest_format = "2.0"
project_hash = "bee765e681e3b3b580bf6543fbb54f52e7d887a4"
