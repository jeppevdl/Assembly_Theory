...
CairoMakie.text!(ax1, 0.5, 140, text = "Pearson correlation: $(round(cor(df.mass, df.ma), digits = 3))", color = :red)
mass_ma
...
CairoMakie.text!(ax2, 0.5, 140, text = "Pearson correlation: $(round(cor(df.counted_atoms, df.ma), digits = 3))", color = :red)
atoms_ma
...
for i in unique(df.counted_rings)
    ...
    CairoMakie.text!(ax3, i, maximum(df.ma) + 3, text = "n=$n", align = (:center, :bottom), color = :black)
end
rings_ma
...
for i in unique(df.sp)
    n = sum(df.sp .== i)
    CairoMakie.text!(ax4, i, maximum(df.ma) + 3, text = "n=$n", align = (:center, :bottom), color = :black)
end
sp_ma
...
CairoMakie.text!(ax5, 0.5, 140, text = "Pearson correlation: $(round(cor(df.sp2, df.ma), digits = 3))", color = :red)
sp2_ma
...
CairoMakie.text!(ax6, 0.5, 140, text = "Pearson correlation: $(round(cor(df.sp3, df.ma), digits = 3))", color = :red)
sp3_ma
