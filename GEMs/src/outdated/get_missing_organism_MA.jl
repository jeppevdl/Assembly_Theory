...
MAs = CSV.read("../data/bash_MA_output/complete_MAs.csv", DataFrame; header=true)
...
for organism in organisms
	...
	open("../data/pathway_complexities/MA_input_$organism", "w") do f
		for id in id_list
			write(f, "$id\n")
		end
	end
...
sort!(calc_ma)
open("../data/pathway_complexities/calc_ma.txt", "w") do f
	for id in calc_ma
		write(f, "$id\n")
	end
end
#save failed_ids to file
open("../data/pathway_complexities/failed_ids.txt", "w") do f
