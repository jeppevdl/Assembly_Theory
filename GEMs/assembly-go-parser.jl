...
# ╔═╡ b8955fdc-2a68-11f0-08f2-454cd6b2f583
begin
...
# ╔═╡ 709e9554-e822-4436-adf2-6a8ef8a17fce
begin # Constants
	order_lut = Dict("single" => 1, "double" => 2, "triple" => 3)
end

# ╔═╡ f6b29bc7-6bde-49b0-8ab1-9e41d93c0850
begin # Dummy values
	V = [1, 4, 8, 10, 11, 12, 13]
	E = [[1 4], [4 8], [8 10], [8 11], [11 12], [11 13]]
	Vc = ["C", "C", "C", "N", "C", "O", "O"]
	Ec = ["single", "single", "single", "single", "single", "double"]
end;

# ╔═╡ 38f710be-dcc5-4488-b638-2ec09cce806e
# Step 1: normalize vertices and edges
begin
	V′ = enumerate(V) .|> reverse |> Dict
	E′ = map(e -> (V′[e[1]], V′[e[2]]), E)
	Vc′ = Vc .|> Symbol .|> SMILESAtom
	Ec′ = map(e -> order_lut[e], Ec)
	V′, E′, Vc′, Ec′
end

# ╔═╡ 398ea98c-c767-4f3f-b288-93b4432f6b8c
# ╠═╡ disabled = true
#=╠═╡
mol = MolGraph()
  ╠═╡ =#

# ╔═╡ 5192d750-331d-48d5-b74c-f10418630643
begin
	mol = smilestomol("") # Empty molecule
	map(a -> add_vertex!(mol, a), Vc′)
	for (e,p) in zip(E′, Ec′)
		add_edge!(mol, e[1], e[2], SMILESBond(p))
	end
	mol
end
...
manifest_format = "2.0"
project_hash = "2e95db2ed2cb72c5e03dc9c9bf473b70b020d3da"
...
[[deps.MolecularGraph]]
deps = ["Base64", "Cairo", "Colors", "Dates", "DelimitedFiles", "GeometryBasics", "Graphs", "JSON", "LinearAlgebra", "MakieCore", "OrderedCollections", "Printf", "Statistics", "YAML", "coordgenlibs_jll", "libinchi_jll"]
git-tree-sha1 = "41dadb9fd65cbaecb503ca25241241c1dcb29dca"
